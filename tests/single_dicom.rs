...
use tracing::debug;
use rudicom::db::{lookup, LocalSessionStream, Session, DB};
...
async fn single_dicom() -> Result<(), Box<dyn std::error::Error>>
{
	...
	if let RegisterResult::Stored(_) = store(obj.clone(), LocalSessionStream::create(&DB, 1)).await? {}
	else { panic!("First store should return stored."); }
	debug!("inserted object");
	if let RegisterResult::AlreadyStored(stored) = store(obj.clone(), LocalSessionStream::create(&DB, 1)).await? {
