use crate::db;
use crate::db::{list_entries, Entry, RecordId};
...
pub async fn get_studies_html(headers: HeaderMap,Query(config): Query<ListingConfig>) -> Result<axum::response::Html<String>, HttpError>
{
	...
	let mut aggregate_collector = JoinSet::new();
	for study in studies.clone() {
		aggregate_collector.spawn(async move {
			match study.get_aggregate().await{
				Ok(v) => Some(v),
				Err(e) => {
					warn!("Failed to aggregate data on {}: {:?}", study.id(),e);
					None
				}
			}
		});
	...
	let aggregate_instances= aggregate_collector.join_all().await;

	// collect results from above
	let instance_count:BTreeMap<RecordId,_> = aggregate_instances.iter()
		.filter_map(|a|a.as_ref().map(|e|(RecordId(e.id.clone()),e.count)))
		.collect();
	let filesizes:BTreeMap<RecordId,_> =aggregate_instances.into_iter()
		.filter_map(|a|a.map(|e|(RecordId(e.id.clone()),Byte::from(e.size)))).collect();
