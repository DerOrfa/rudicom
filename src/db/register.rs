...
-> Result<sql::Value>
{
	...
	res.take::<Value>(2)
		.and_then(|v|
			match v {
				Value::None | Value::Null => Ok(Value::None),
				Value::Array(mut array) => {
					Ok(
						if array.len() ==1 {array.remove(0)}
						else {Value::Array(array)}
					)
				}
				Value::Object(object) => Ok(Value::Object(object)),
				_ => { Err(surrealdb::error::Api::ParseError("expected array or object".to_string()).into()) }
			}
		)
}
...
) -> anyhow::Result<Option<db::Entry>>
{
	...
	let res=register(instance_meta,series_meta,study_meta).await?.flatten();
